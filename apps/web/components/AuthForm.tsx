...
export default function AuthForm({ clientManager }:{ clientManager:ClientManager }) {
    ...
    async function signUpForAccount() {
        clientManager._session._signUp({ email, password, username })
            .then(() => {
    ...
    return (
        <div className={`h-full min-h-screen w-full transition-all`}>
            ...
            <div className={`bg-background transition-all md:bg-transparent`}>

                {/* <div className={`w-full fixed h-20 text-white flex items-center px-7`}>
                    <h1 className={`text-3xl font-black`}>Bonmelo</h1>
                </div> */}

                <div className={`w-full min-h-screen h-full flex pt-6 md:pt-0 md:items-center transition-all md:justify-center md:px-16`}>
                    <motion.div
                        animate={loginDialogAnimationControl}
                        initial={{ translateY: -100, opacity: 0 }}
                        transition={{ ease: 'anticipate', duration: 0.3 }}
                        className={`bg-background text-foreground w-full h-full transition-all ${isSignIn ? `md:w-[650px] lg:w-[850px]` : `md:w-[550px] lg:w-[650px]`} rounded-2xl md:shadow-xl md:outline outline-[3px] md:outline-indigo-700 py-5 px-5`}
                    >
                        <div className={`flex items-center ${showEmailVerification ? 'hidden' : 'block'}`}>
                            <div className={`w-full`}>
                                <h1 className={`font-bold text-2xl text-[#19114D]`}>{`Welcome back! 👋`}</h1>
                                <h3 className={`text-[13px] text-[#110b33] font-medium`}>Lets sign you into Bonmelo!</h3>

                                <div className={`mt-7 space-y-3`}>

                                    <div id="usernameForm" className={`${isSignIn ? 'hidden' : 'block'}`}>
                                        <h1 className={`text-gray-600 text-sm`}>Username</h1>
                                        <input
                                            placeholder="Enter your username."
                                            type={`text`}
                                            onChange={(ev) => setUsername(ev.currentTarget.value)}
                                            className={`text-black bg-[#F1F3F5] mt-2 w-full px-4 py-2 rounded-lg focus:outline outline-2 outline-[#339AF0]`}
                                        />
                                    </div>

                                    <div id="emailForm">
                                        <h1 className={`text-gray-600 text-sm font-medium`}>Email Address</h1>
                                        <input
                                    ...
                                    <div id="passwordForm">
                                        <h1 className={`text-gray-600 text-sm font-medium`}>Password</h1>
                                        {/* <input
                                        ...
                                        /> */}
                                        <PasswordInput
                                            className={`mt-2 bg-blue-50 rounded-lg`}
                                            onChange={(ev) => setPassword(ev.currentTarget.value)}
                                ...
                                <div className={`mt-3`}>
                                    <button onClick={() => isSignIn ? signInToAccount() : signUpForAccount()} className={`bg-black bg-opacity-20 hover:bg-indigo-300 font-semibold rounded-xl transition-all mt-4 w-full py-2`}>
                                        <h1 className={'text-white'}>{isSignIn ? 'Login' : 'Register'}</h1>
                                    </button>
                                </div>

                                <div className={`text-sm mt-5 space-y-1`}>
                                    <div className={`flex space-x-1`}>
                                        <h1 className={`text-black`}>{isSignIn ? `New to Bonmelo?` : `Already have an account?`}</h1>
                                        <button onClick={() => setIsSignIn(!isSignIn)} className={`text-red-400`}>{isSignIn ? `Create a new account` : 'Login'}</button>
                                    </div>

                                    <div className={`flex space-x-1`}>
                                        <h1 className={`text-black`}>Forgot your password?</h1>
                                        <button className={`text-red-400`}>Reset password</button>
                                    </div>
                                </div>
                            </div>

                            <div className={`${isSignIn ? 'hidden md:flex' : 'hidden'} flex-col text-center justify-center items-center h-full w-64 ml-10 mr-2 text-[#19114D]`}>
                                <canvas
                                    id="qrCodeCanvas"
