...
import { AddCircle, CallOutline, SearchOutline } from 'react-ionicons'
...
import { Badge, LoadingOverlay, Menu, Skeleton } from '@mantine/core';
...
import { MoodSmile, ThreeDCubeSphere } from 'tabler-icons-react';
...
export default function ChannelView({ clientManager }:AppProps) {
    ...
    async function sendAction(event) {
        if (event?.key == 'Enter' && !event?.shiftKey) {
            ...
            const channelClient = await getChannelClient()
                    ...
                    <div className={`w-full h-full flex`}>
                        ...
                        <div id="chatArea" className={`overflow-hidden bg-center bg-cover bg-no-repeat h-full w-full text-black`}>
                            ...
                            <div id="topToolBar" className={`bg-white z-50 h-12 backdrop-saturate-150 backdrop-brightness-[0.98] px-7 w-full relative flex justify-between items-center shadow-sm`}>
                                ...
                                <div className={`flex items-center`}>
                                    <div className={`space-x-3 flex items-center`}>
                                        <button>
                                            <CallOutline
                                                cssClasses={`text-[#575861] dark_p:text-white`}
                                                width={`23px`}
                                                height={`23px`}
                                            />
                                        </button>
                            ...
                            <div className={`relative h-full -mt-[77px]`}>
                                <div id="chatContent" className={`overflow-x-hidden relative overflow-y-scroll w-full h-full pb-14 pt-[77px]`}>
                                    ...
                                    {(() => {
                                        ...
                                        return (messages[channelData?.id as string] || [])?.slice()?.sort((a?, b?) => (Number(a?.timestamp) || 0) - (Number(b?.timestamp) || 0))?.map((unbuiltMessage) => {
                                            ...
                                            const Container = ({ children }) => (
                                                <div className={`${clientManager?.developmentMode == true ? 'cursor-pointer' : ''}`} id={`messageContainer_${message?.id}`} onClick={() => messageContainerClick()}>
                                                    {children}
                                                </div>
                                            )
                                ...
                                <div className={`w-full relative h-fit`}>
                                    <div className={'max-w-full absolute -bottom-5 left-5 right-7'}>
                                        <div className={`w-full h-fit bg-yellow-50 shadow-md rounded-2xl flex items-center px-3 py-2`}>
                                            <div>
                                                <button className={`flex items-center`}>
                                                    <AddCircle />
                                                </button>
